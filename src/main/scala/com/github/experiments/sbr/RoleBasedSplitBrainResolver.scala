package com.github.experiments.sbr
...
object RoleBasedSplitBrainResolver {
  def props(stableAfter: FiniteDuration, essentialRoles: Set[String]): Props =
    Props(new RoleBasedSplitBrainResolver(stableAfter, essentialRoles))
...
class RoleBasedSplitBrainResolver(stableAfter: FiniteDuration, essentialRoles: Set[String]) extends Actor with ActorLogging {
  ...
  val ignoreMemberStatus: Set[MemberStatus] = Set[MemberStatus](Down, Exiting)
  ...
  def nodesAddress: SortedSet[UniqueAddress] = members.map(_.uniqueAddress)

  def reachableNodesAddress: SortedSet[UniqueAddress] = members.collect {
    case m if !unreachable(m.uniqueAddress) => m.uniqueAddress
  }

  def unreachableMembers: SortedSet[Member] = members.filter(m => unreachable(m.uniqueAddress))

  def reachableMembers: SortedSet[Member] = members.filter(m => !unreachable(m.uniqueAddress))

  override def preStart(): Unit = {
    log.info("Starting Split-Brain-Resolver")
